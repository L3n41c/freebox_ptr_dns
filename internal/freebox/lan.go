...
func (c *Client) ListInterfaces(ctx context.Context) ([]LanInterface, error) {
	...
	if err := c.doAuth(ctx, "GET", "/api/v4/lan/browser/interfaces/", nil, &ifaces); err != nil {
		return nil, err
	}
	return ifaces, nil
}
...
func (c *Client) ListHosts(ctx context.Context, iface string) ([]LanHost, error) {
	var hosts []LanHost
	if err := c.doAuth(ctx, "GET", fmt.Sprintf("/api/v4/lan/browser/%s/", iface), nil, &hosts); err != nil {
		return nil, err
	}
	return hosts, nil
}
